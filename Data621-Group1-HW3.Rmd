...
par(mfrow = c(1,1))
```
The plot on the left shows that the number of predictors with the lowest BIC are `nox` , `age`, `rad`, and `medv`. We will use those predictors to build the next model
```{r}
